...
bool ProtocolLogin::parseFirstPacket(NetworkMessage& msg)
{
	...
	uint32_t id = 1;
	if(!IOLoginData::getInstance()->getAccountId(name, id)) //CA
	{
	...
	if(!encryptTest(account.salt + password, account.password)) //CA
	{
		ConnectionManager::getInstance()->addAttempt(clientIp, protocolId, false);
	...
	if(!g_config.getBool(ConfigManager::ACCOUNT_MANAGER) && !account.charList.size()) //CA
	{
